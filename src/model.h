...
private:
    ...
    // Used for Environment assets (Grass) -> NO aiProcess_FlipUVs
        ...
        // Used for Mixamo Characters -> WITH aiProcess_FlipUVs
    ...
    Mesh processMesh(aiMesh *mesh, const aiScene *scene)
    {
        ...
        // 1. diffuse / base color maps (FBX PBR models use BASE_COLOR instead of DIFFUSE)
        vector<Texture> diffuseMaps = loadMaterialTextures(material, aiTextureType_DIFFUSE, "texture_diffuse", scene);
        if (diffuseMaps.empty())
            diffuseMaps = loadMaterialTextures(material, aiTextureType_BASE_COLOR, "texture_diffuse", scene);
        if (diffuseMaps.empty())
            diffuseMaps = loadMaterialTextures(material, aiTextureType_EMISSIVE, "texture_diffuse", scene); // Extra fallback for Collada
